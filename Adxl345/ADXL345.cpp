...
void ADXL345::begin() {
	...
	write(ADXL345_POWER_CTL, 0);      
	write(ADXL345_POWER_CTL, 16);
...
void ADXL345::readAccel(int *x, int *y, int *z) {
	read(ADXL345_DATAX0, ADXL345_TO_READ, _buff); //read the acceleration data from the ADXL345
...
void ADXL345::read(byte address, int num, byte _buff[]) {
	Wire.beginTransmission(ADXL345_DEVICE); // start transmission to device 
...
void ADXL345::setAxisOffset(int x, int y, int z) {
	write(ADXL345_OFSX, byte (x));  
	write(ADXL345_OFSY, byte (y));  
	write(ADXL345_OFSZ, byte (z));  
}

// Gets the OFSX, OFSY and OFSZ bytes
void ADXL345::getAxisOffset(int* x, int* y, int*z) {
	byte _b;
	read(ADXL345_OFSX, 1, &_b);  
	*x = int (_b);
	read(ADXL345_OFSY, 1, &_b);  
	*y = int (_b);
	read(ADXL345_OFSZ, 1, &_b);  
	*z = int (_b);
}
...
void ADXL345::setActivityThreshold(int activityThreshold) {
	...
	write(ADXL345_THRESH_ACT, _b);  
}

// Gets the THRESH_ACT byte
int ADXL345::getActivityThreshold() {
	byte _b;
	read(ADXL345_THRESH_ACT, 1, &_b);  
	return int (_b);
}
...
void ADXL345::setInactivityThreshold(int inactivityThreshold) {
	...
	write(ADXL345_THRESH_INACT, _b);  
}

// Gets the THRESH_INACT byte
int ADXL345::getInactivityThreshold() {
	byte _b;
	read(ADXL345_THRESH_INACT, 1, &_b);  
	return int (_b);
}
...
void ADXL345::setTimeInactivity(int timeInactivity) {
	...
	write(ADXL345_TIME_INACT, _b);  
}

// Gets the TIME_INACT register
int ADXL345::getTimeInactivity() {
	byte _b;
	read(ADXL345_TIME_INACT, 1, &_b);  
	return int (_b);
}
...
void ADXL345::setFreeFallThreshold(int freeFallThreshold) {
	...
	write(ADXL345_THRESH_FF, _b);  
}

// Gets the THRESH_FF register.
int ADXL345::getFreeFallThreshold() {
	byte _b;
	read(ADXL345_THRESH_FF, 1, &_b);  
	return int (_b);
}
...
void ADXL345::setFreeFallDuration(int freeFallDuration) {
	...
	write(ADXL345_TIME_FF, _b);  
}

// Gets the TIME_FF register.
int ADXL345::getFreeFallDuration() {
	byte _b;
	read(ADXL345_TIME_FF, 1, &_b);  
	return int (_b);
}
...
void ADXL345::set_bw(byte bw_code){
	if((bw_code < ADXL345_BW_3) || (bw_code > ADXL345_BW_1600)){
		...
		write(ADXL345_BW_RATE, bw_code);
	}
}

byte ADXL345::get_bw_code(){
	byte bw_code;
	read(ADXL345_BW_RATE, 1, &bw_code);
	return bw_code;
}
